...
public class ShopArr {
	...
	private void intro(Scanner input) {
		...
		for(int i = 0; i < cheese.length; i++){
		System.out.print("Enter amount of " + cheese[i].name + " $ " + cheese[i].price  + " per pound: ");
		int max = input.nextInt();
		cheese[i].setAmount(max);
		// Fill-in Code
		}
	}
	...
	private void itemizedList(){
		...
		for (int i = 0; i < cheese.length; i++){
			if ((amt = cheese[i].getAmount()) > 0) 
				System.out.println(amt + " lbs of " + cheese[i].name + cheese[i].price + " = " + (amt * cheese[i].getPrice()));	
		}
	}
